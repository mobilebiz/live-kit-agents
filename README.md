...
エージェントの動作は `instructions.md` ファイルで制御できます。

### 指示の変更方法

1. `instructions.md` を編集してエージェントの振る舞いを変更
2. エージェントを再起動（`npm run dev`）

### 例

```markdown
あなたは親切なAIアシスタントです。日本語で話してください。
ユーザーの質問に丁寧に答えてください。
天気の質問にはツールを使って答えてください。
```
...
*   `instructions.md`: エージェントへの指示（カスタマイズ可能）
*   `tools/`: 外部ツール定義ディレクトリ
